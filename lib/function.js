...
const { regExpFromString } = require('./utils');
...
const builtInsImpls = {
  ...
  String: {
    ...
    match: (target, context, options, runState) => {
      ...
      return target.match(regExpFromString(pattern));
    },
    ...
    search: (target, context, options, runState) => {
      const pattern = context.get('pattern');
      return target.search(regExpFromString(pattern));
    },
  },
  JSON: {
