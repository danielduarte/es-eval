...
class GenericFunction extends AbstractFunction { // @todo generalize class name to all functions, not only lambdas
  ...
  exec(params, context) {
    ...
    switch (this.body.type) {
      case NODE_BLOCK_STATEMENT:     return evalBlockStatement(this.body, fnContext, this.thisTarget).result;
  ...
  Array: {
    ...
    reduce: (target, context) => {
      ...
      if (context.isDefined('initialValue')) {
        ...
        return target.reduce((acc, elem, i) => fn.exec([acc, elem, i, target], context), initial);
      } else {
        return target.reduce((acc, elem, i) => fn.exec([acc, elem, i, target], context));
      }
    },
  },
  Object: {
