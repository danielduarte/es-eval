...
class AbstractFunction {
  ...
  toString() {
    ...
    // @todo add test for this default method
...
const builtIns = {
  Array: {
    ...
    // Available
    concat:      new GenericFunction({ type: NODE_BUILTIN_METHOD_IMPL, targetClass: 'Array', name: 'concat'      }, [{ name: 'valueN' }], undefined, { isVariadic: true }),
    ...
    join:        new GenericFunction({ type: NODE_BUILTIN_METHOD_IMPL, targetClass: 'Array', name: 'join'        }, [{ name: 'separator' }]),
    ...
    sort:        new GenericFunction({ type: NODE_BUILTIN_METHOD_IMPL, targetClass: 'Array', name: 'sort'        }, [{ name: 'compareFn' }]),
...
const builtInsImpls = {
  Array: {
    ...
    concat: (target, context, options, runState) => {
      return target.concat(...context.get('valueN'));
    },
    join: (target, context, options, runState) => {
      return target.join(context.get('separator'));
    },
    sort: (target, context, options, runState) => {
      const compareFn = context.get('compareFn');
      if (typeof compareFn !== 'undefined') {
        const isFunc = compareFn instanceof AbstractFunction;
        assert(isFunc, `Function expected`);
        return target.sort((a, b) => compareFn.exec([a, b], context, options, runState));
      } else {
        return target.sort();
      }
    },
