...
const evalSpreadElement = (node, context, target, options, runState) => {
  ...
  if (argument.type === NODE_ARRAY_EXPRESSION) {
    return evalArrayExpression(argument, context, target, options, runState);
  } else {
    return evalObjectExpression(argument, context, target, options, runState)
  }
};
