...
const evalIdentifier = (node, context) => {
  ...
  const defined = Object.prototype.hasOwnProperty.call(context, name);
  if (defined) {
    // If any of 'undefined', 'Infinity', 'NaN' were overridden in current context, the user value is supplied
    return context[name]; // Here the secure access to the property is ensured by having secure variables in context: the ones this evaluator creates.
  }
  ...
  switch (name) {
    ...
    case NAN: return NaN;
    default: assert.fail(`Identifier not defined: '${name}'`);
  }
};
...
const evalCallExpression = (node, context) => {
  ...
  const calleeFn = evalExpressionNode(callee, context, [TYPE_NUMBER, TYPE_BOOLEAN]);
  const argumentsResults = callArguments.map(exp => evalExpressionNode(exp, context, [TYPE_NUMBER, TYPE_BOOLEAN]));

  return calleeFn.exec(argumentsResults);
};
...
const evalExpressionNode = (node, context, typeNames) => {
  switch (node.type) {
    ...
    case NODE_CALL_EXPRESSION: return evalCallExpression(node);
