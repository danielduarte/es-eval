...
const evalReturnStatement = (node, context, target) => {
  ...
  return evalExpressionNode(argument, context, target);
};
...
const evalStatementNode = (node, context, target) => {
  switch (node.type) {
    ...
    default: assert.fail(`Expected one of: '${NODE_VARIABLE_DECLARATION}', ${NODE_BLOCK_STATEMENT}', '${NODE_RETURN_STATEMENT}', '${NODE_EXPRESSION_STATEMENT}', '${NODE_EMPTY_STATEMENT}', but given '${node.type}'`);
  }
};
...
const evalBlockStatement = (node, context, target) => {
  ...
  while (instructionPointer < body.length && !breakExecution) {
    ...
    const statementResult = evalStatementNode(statement, context, target);
...
const evalCallExpression = (node, context, target) => {
  ...
  assert(calleeFn instanceof implClasses.AbstractFunction, 'Not a function');
...
const evalMemberExpressionNode = (node, context, target) => {
  ...
  assert(typeof objectResult === 'object', `Expected object but given ${typeof objectResult}`); // @todo support member expressions for non-objects
...
const evalExpressionStatement = (node, context, target) => {
  ...
  return evalExpressionNode(expression, context, target);
};
...
const evalProgram = (node, context) => {
  ...
  return evalExpressionStatement(statement, context, globalThis);
};
