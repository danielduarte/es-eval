...
const evalCallExpression = (node, context) => {
  ...
  const calleeFn = evalExpressionNode(callee);
...
const evalAssignmentExpression = (node, context) => {
  ...
  if (left.type === NODE_IDENTIFIER) {
    ...
    const { object, name } = evalMemberExpressionNode(left, context);
...
const evalMemberExpression = (node, context) => {
  const { object, name } = evalMemberExpressionNode(node, context);
...
const evalMemberExpressionNode = (node, context) => {
  ...
  assert(typeof objectResult === 'object');
  // assert(objectResult !== null); // @todo add when supporting null
  assert(!Array.isArray(objectResult));
  ...
  return { object: objectResult, name };
};
