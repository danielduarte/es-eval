...
const evalStatementNode = (node, context, target) => {
  switch (node.type) {
    ...
    case NODE_WHILE_STATEMENT: return evalWhileStatement(node, context, target);
...
const evalWhileStatement = (node, context, target) => {
  ...
  while (!statResult.exit && evalExpressionNode(test, context, target)) {
    statResult = evalStatementNode(body, context, target);
  }
