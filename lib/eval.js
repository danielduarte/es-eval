...
const supportedUnaryOps = {
  ...
  'typeof': x => typeof x,
  'void': x => void x,
  'delete': x => [TYPE_NUMBER, TYPE_STRING, TYPE_BOOLEAN, TYPE_OBJECT, TYPE_FUNCTION].includes(typeof x), // @todo add test deleting object properties
};
...
const evalBuiltInMethodImpl = (node, context, target, options, runState) => {
  ...
  return builtInsImpl[targetClass][name](target, context, options, runState);
};
...
const evalCallExpression = (node, context, target, options, runState) => {
  ...
  assert(calleeFn instanceof implClasses.AbstractFunction, typeof calleeFn !== 'function' ? `Function expected but given ${typeof calleeFn}` : `Cannot access a user provided function`);
...
const evalAssignmentExpression = (node, context, target, options, runState) => {
  ...
  // const leftResult = evalExpressionNode(left, context, target, options); // @todo will be required for other kind of assignments
