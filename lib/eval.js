...
const NODE_FOROF_STATEMENT = 'ForOfStatement';
...
const supportedUnaryOps = {
  ...
  // 'delete' operator to delete object properties is a special case considered in 'evalUnaryExpression()'
...
const evalStatementNode = (node, context, target, options, runState) => {
  ...
  switch (node.type) {
    ...
    case NODE_FOROF_STATEMENT:      { result = evalForOfStatement(node, context, target, options, runState); break; }
    ...
    default: assert.fail(`Expected one of: '${NODE_VARIABLE_DECLARATION}', ${NODE_BLOCK_STATEMENT}', ${NODE_IF_STATEMENT}', '${NODE_FOROF_STATEMENT}', '${NODE_WHILE_STATEMENT}', '${NODE_RETURN_STATEMENT}', '${NODE_EXPRESSION_STATEMENT}', '${NODE_EMPTY_STATEMENT}', but given '${node.type}'`);
  }
...
const evalBlockStatement = (node, context, target, options, runState) => {
  ...
  // @todo Check if a new child context should be created. The same in other block statements (loops, if, etc.).
...
const evalForOfStatement = (node, context, target, options, runState) => {
  assert.strictEqual(node.type, NODE_FOROF_STATEMENT);

  const { await, left, right, body } = node;
  assert.strictEqual(await, false, "'for await' construction is not supported yet");

  let statResult;

  // @todo add support for iterations over variables already declared or over object fields.
  assert.strictEqual(left.type, NODE_VARIABLE_DECLARATION, "'for of' loop supports only variable declarations in this version");
  let loopContext = new Context({}, context);
  statResult = evalVariableDeclaration(left, loopContext, target, options, runState);
  const loopVars = Object.entries(loopContext.asObject(true));
  assert.strictEqual(loopVars.length, 1);
  const [[loopVarId]] = loopVars;

  const isConst = loopContext.getDef(loopVarId).isConst;

  // @todo add support for iterators in general being careful with the function call `iterator[Symbol.iterator]()`.
  const iterable = evalExpressionNode(right, context, target, options, runState);
  assert.strictEqual(Array.isArray(iterable), true, "'for of' loop supports only iteration over arrays in this version");
  for (let i = 0; !statResult.exit && i < iterable.length; i++) {
    const currentValue = iterable[i];
    loopContext = new Context({ [loopVarId]: { value: currentValue, isConst } }, context);
    statResult = evalStatementNode(body, loopContext, target, options, runState);
  }
...
const createFunction = (node, context, target, isLambda) => {
  ...
  const paramDefs = params.map(param => ({ name: evalIdentifierNode(param) })); // @todo(feat) implement array destructuring and not only expect evalIdentifierNode here
...
const evalMemberExpression = (node, context, target, options, runState) => {
  ...
  const builtInType = isArray ? 'Array' : (typeof object === 'string' ? 'String' : 'Object'); // @todo(refactor) move this logic to evalMemberExpressionNode
  ...
  // Supports access to own properties only
...
const evalUnaryExpression = (node, context, target, options, runState) => {
  ...
  if (operator === 'delete' && argument.type === NODE_MEMBER_EXPRESSION) {
    // @todo delete array[3]
    const { object, name, isArray } = evalMemberExpressionNode(argument, context, target, options, runState);

    assert(typeof object !== 'undefined', `Cannot convert undefined to object`);
    assert(object !== null, `Cannot convert null to object`);
    const builtInType = isArray ? 'Array' : 'Object';
    const isBuiltInProperty = Object.prototype.hasOwnProperty.call(builtIns[builtInType], name);
    if (isBuiltInProperty) {
      let member = builtIns[builtInType][name];
      return member instanceof implClasses.AbstractFunction; // Trying to delete built-in properties doesn't do anything. Returns true for functions (methods) and false otherwise.
    }
    if (typeof object !== 'object') { return false; } // @todo(chore) review this case
    const isOwnProperty = Object.prototype.hasOwnProperty.call(object, name);
    return isOwnProperty ? delete object[name] : true;
  }
  ...
  if (argumentResult instanceof implClasses.AbstractFunction) {
    ...
    argumentResult = () => {}; // All unary operators returns the same result for all functions
  }
...
const evalProgram = (node, context, target, options, runState) => {
  ...
  const { result } = evalExpressionStatement(statement, context, target, options, runState);
