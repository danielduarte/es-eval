...
const supportedUnaryOps = {
  ...
  'typeof': x => typeof x,
  'void': x => void x,
  'delete': x => [TYPE_NUMBER, TYPE_STRING, TYPE_BOOLEAN, TYPE_OBJECT, TYPE_FUNCTION].includes(typeof x), // @todo(test) add test deleting object properties
};
...
const evalArrowFunctionExpression = (node, target) => {
  ...
  // @todo(chore) double check meaning of node.expression
  ...
  return new implClasses.GenericFunction(body, paramDefs, false, true, target);
};

// @todo(refactor) unify with evalArrowFunctionExpression?
const evalFunctionExpression = (node, target) => {
  ...
  return new implClasses.GenericFunction(body, paramDefs, false, false, target);
};
...
const evalCallExpression = (node, context, target, options, runState) => {
  ...
  const argumentsResults = callArguments.map(exp => evalExpressionNode(exp, context, target, options, runState));
...
const evalProperty = (node, context, target, options, runState) => {
  ...
  assert.strictEqual(method, false); // @todo(chore) double check meaning of field 'method'
...
const evalArrayExpression = (node, context, target, options, runState) => {
  ...
  return elements.map(element => evalExpressionNode(element, context, target, options, runState));
};
...
const evalMemberExpression = (node, context, target, options, runState) => {
  ...
  if (member instanceof implClasses.AbstractFunction) { // For security reasons, do not remove this check
    ...
    assert(typeof member !== TYPE_FUNCTION, `Cannot access a user provided functions`);
  }
...
const evalMemberExpressionNode = (node, context, target, options, runState) => {
  ...
  assert(['object', 'number', 'string', 'undefined'].includes(typeof objectResult), `Unexpected value of type ${typeof objectResult}`); // @todo(feat) support member expressions for non-objects
...
const evalExpressionNode = (node, context, target, options, runState) => {
  switch (node.type) {
    ...
    case NODE_ARROW_FUNCTION_EXPRESSION: return evalArrowFunctionExpression(node, target); // @todo(chore) context not needed? (closures)
    case NODE_FUNCTION_EXPRESSION:       return evalFunctionExpression(node, target); // @todo(chore) context not needed? (closures)
