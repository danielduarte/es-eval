...
const evalStatementNode = (node, context, target) => {
  switch (node.type) {
    ...
    default: assert.fail(`Expected one of: '${NODE_RETURN_STATEMENT}', '${NODE_EXPRESSION_STATEMENT}', '${NODE_VARIABLE_DECLARATION}', '${NODE_EMPTY_STATEMENT}', but given '${node.type}'`);
  }
};
